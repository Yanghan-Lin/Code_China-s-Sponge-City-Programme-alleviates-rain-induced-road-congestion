...
* File:    12_supplementary_table7_robustness_checks.do
...
log using "${LOGS}/12_supplementary_table7_robustness_checks.log", replace text
...
* (3) Controlling for COVID-19 intensity
...
quietly reghdfe tcdi_im policy covid_intensity_100 ///
        if `MAIN_SAMPLE' & !missing(covid_intensity_100), ///
        absorb(city_code month_id) vce(cluster city_code)
    estadd local city_fe "Yes"
    estadd local ym_fe "Yes"
    eststo S7_covid_intensity

* (4) Excluding local or provincial SCP pilots from the control group
quietly reghdfe tcdi_im policy ///
...
* (5) Controlling for concurrent climate-adaptation policies
...
* (6) Accounting for non-random policy selection
...
* (7) Stricter traffic and transport controls
quietly reghdfe tcdi_im ///
...
* (9) City-by-year clustered standard errors
quietly reghdfe tcdi_im policy ///
...
* (10) Province clustered standard errors
quietly reghdfe tcdi_im policy ///
...
* (11) Province-by-year clustered standard errors
quietly reghdfe tcdi_im policy ///
...
esttab ///
    S7_baseline S7_covid S7_covid_intensity S7_local_scp S7_confounding ///
    ...
    mtitles("Baseline" "Excluding COVID-19" "COVID-19 intensity control" "Excluding local SCP pilots" ///
    ...
    keep(policy covid_intensity_100 climate_policy ln_taxi_panel ln_bus_panel ln_private_car_panel ///
         ln_public_transport_inv ln_road_bridge_inv) ///
    order(policy covid_intensity_100 climate_policy ln_taxi_panel ln_bus_panel ln_private_car_panel ///
          ln_public_transport_inv ln_road_bridge_inv) ///
